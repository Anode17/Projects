...
wine_revs<-read_csv("winemag-data-130k-v2.csv") %>% data.table()
```
## Introduction.  
...
wine_revs<-rename(wine_revs,V1=...1)
...
```{r sentiment-scores-for-words,echo=FALSE}
...
```{r grouping-and-casting,echo=FALSE,message=FALSE,warning=FALSE,results='hide'}
...
subRegion2Column<-function(string){
  ...
  #set(wine_revs,j=paste0(tolower(string)),ifelse(region_2==string,1,0))
}
...
data=
  wine_revs %>%
  select(
    -c(country,V1,designation,description,point_cats,province,region_1,region_2,taster_name,taster_twitter_handle,variety,winery,title,`Other region`,`Other variety`,acceptable,"NA"))
...
```{r models,echo=FALSE,warning=FALSE,message=FALSE,error=FALSE,cache=TRUE}
##glm models
...
```{r last,echo=FALSE,results='asis'}
stargazer(rbind(train_pred,test_pred),title="RMSE",summary=F,out.header = F, header = F)
```
